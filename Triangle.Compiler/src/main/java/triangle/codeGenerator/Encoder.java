...
public final class Encoder implements ActualParameterVisitor<Frame, Integer>,
	...
	public Void visitLoopWhileCommand(LoopWhileCommand ast, Frame frame) {
		...
		ast.C1.visit(this, frame);
		ast.E.visit(this, frame);
		var endJump=emitter.emit(OpCode.JUMPIF, Machine.falseRep, Register.CB, loopAddr);
		ast.C2.visit(this, frame);
		emitter.emit(OpCode.JUMP, Machine.falseRep, Register.CB, loopAddr);
		emitter.patch(endJump);
		return null;
	}
	...
	private final void elaborateStdEnvironment() {
		...
		elaborateStdPrimRoutine(StdEnvironment.barDecl, Primitive.ID);
	}
